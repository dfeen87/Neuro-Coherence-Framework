...
class NeuroCoherence:
    ...
    def __init__(self, phi: float = 1.0, integration_method: str = "trapezoidal"):
        ...
        if integration_method not in ["trapezoidal", "simpson", "mean"]:
            raise ValueError("integration_method must be 'trapezoidal', 'simpson', or 'mean'")
    ...
    def calculate(
        ...
        if time is not None or space is not None or energy is not None:
            ...
            psi_value = self._integrate_multidim(
                integrand, time, space, energy
            )
        else:
    ...
    def _extract_value(self, val: Union[OperatorResult, float, np.ndarray]) -> np.ndarray:
    ...
    def _integrate_simple(self, integrand: np.ndarray) -> float:
        ...
        if self.integration_method == "mean":
            ...
            if integrand.ndim == 1 and len(integrand) > 1:
                ...
                return float(np.mean(integrand))  # Fallback for single value or multidim
        else:  # simpson
    ...
    def simulate_bipolar_episode(
        ...
        if episode_type not in ["manic", "depressive", "euthymic"]:
            raise ValueError("episode_type must be 'manic', 'depressive', or 'euthymic'")
        ...
        if episode_type == "manic":
            ...
            homeostasis = 0.5 + 0.2 * np.sin(0.1 * time) + 0.1 * np.random.randn(duration)
            sync_variance = 0.6 + 0.2 * np.random.randn(duration)
            phase_alignment = 0.4 + 0.3 * np.sin(0.05 * time) + 0.1 * np.random.randn(duration)
    ...
    def run_parameter_sweep(
        ...
        for i, param_val in enumerate(param_range):
            ...
            gamma = self.gamma_op.calculate(
                params.get("plasticity", 0.8),
                params.get("responsiveness", 0.5)
            )
            theta = self.theta_op.calculate(
                params.get("homeostasis", 0.9)
            )
            delta = self.delta_op.calculate(
                params.get("sync_variance", 0.3)
            )
            lambda_op = self.lambda_op.calculate(
                params.get("phase_alignment", 0.85)
            )
