...
def main():
    ...
    euthymic = nc.simulate_bipolar_episode(
        duration=200, episode_type="euthymic", dt=0.1
    )
    print(f"   Euthymic Ψ (mean): {np.mean(euthymic['psi']):.4f}")

    # Manic episode
    manic = nc.simulate_bipolar_episode(duration=200, episode_type="manic", dt=0.1)
    print(f"   Manic Ψ (mean): {np.mean(manic['psi']):.4f}")

    # Depressive episode
    depressive = nc.simulate_bipolar_episode(
        duration=200, episode_type="depressive", dt=0.1
    )
    ...
    recovery = nc.simulate_recovery_curve(
        initial_psi=0.3, target_psi=0.85, duration=200, recovery_rate=0.02
    )
    ...
    ax2.axhline(
        y=recovery["initial_psi"], color="blue", linestyle="--", label="Initial"
    )
    ax2.set_xlabel("Time")
