            ...
            if i != j:
                for k in range(n):
                    if k != i and k != j:
                        ...
                        if (np.isfinite(dist_matrix[i, k]) and 
                            np.isfinite(dist_matrix[k, j]) and
                            abs(dist_matrix[i, j] - (dist_matrix[i, k] + dist_matrix[k, j])) < 1e-10):
                            betweenness[k] += 1
...
def calculate_rich_club_coefficient(
    conn_matrix: np.ndarray,
    k: int
) -> float:
    """
