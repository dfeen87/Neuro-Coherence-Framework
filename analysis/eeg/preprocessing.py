...
def remove_baseline(
    signal: np.ndarray, baseline_window: Optional[Tuple[int, int]] = None
) -> np.ndarray:
    """
...
def detect_artifacts(signal: np.ndarray, threshold: float = 3.0) -> np.ndarray:
    """
...
def interpolate_artifacts(signal: np.ndarray, artifacts: np.ndarray) -> np.ndarray:
    """
    ...
    signal_clean[artifact_indices] = np.interp(
        artifact_indices, good_indices, signal[good_indices]
    )
...
def apply_notch_filter(
    signal: np.ndarray, fs: float, freq: float = 60.0, quality: float = 30.0
) -> np.ndarray:
    """
