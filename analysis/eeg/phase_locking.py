...
def calculate_plv(
    signal1: np.ndarray, signal2: np.ndarray, window_size: Optional[int] = None
) -> float:
    """
...
def calculate_plv_matrix(
    signals: np.ndarray, window_size: Optional[int] = None
) -> np.ndarray:
    """
...
def extract_frequency_band(
    signal: np.ndarray, fs: float, band: str = "alpha"
) -> np.ndarray:
    """
...
def calculate_phase_coherence(
    signal1: np.ndarray, signal2: np.ndarray, fs: float, band: Optional[str] = None
) -> float:
    """
