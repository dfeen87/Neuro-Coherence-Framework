...
def run_perturbation_experiment():
    ...
    baseline = nc.simulate_bipolar_episode(
        duration=500, episode_type="euthymic", dt=0.1
    )
    ...
    for t in perturbation_times:
        perturbed = nc.apply_perturbation(
            baseline_trajectory=baseline, perturbation_time=t, strength=0.6, duration=50
        )
...
def run_parameter_sensitivity():
    ...
    for param_name, param_range in parameters.items():
        ...
        result = nc.run_parameter_sweep(
            param_name=param_name, param_range=param_range, fixed_params=fixed_params
        )
...
def run_recovery_comparison():
    ...
    for rate, color in zip(recovery_rates, colors):
        recovery = nc.simulate_recovery_curve(
            initial_psi=0.25, target_psi=0.85, duration=300, recovery_rate=rate
        )
    ...
    ax.axhline(
        y=0.85, color="red", linestyle="--", linewidth=1, alpha=0.5, label="Target"
    )
    ax.axhline(
        y=0.25, color="blue", linestyle="--", linewidth=1, alpha=0.5, label="Initial"
    )
